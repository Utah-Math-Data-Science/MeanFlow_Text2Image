...
def make_targets(txt_tokens: torch.Tensor, img_tokens: torch.Tensor, t: torch.Tensor, eps: float = 1e-6) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    return z_t, v, torch.ones_like(txt_tokens)  # target velocity, t is always positive
        ...
        for step, (imgs, captions) in enumerate(loader):
            ...
            def loss_fn(img_tok, txt_tok, r_, t):
                ...
                v_pred, dvdt = jvp_fn(u_fn,
                                     (z_t, r_, t),
                                     (v.detach(), torch.zeros_like(r_), 
                                      torch.ones_like(t))
                                      )  
                return v_pred, dvdt
            eps = torch.randn_like(txt_tok)
            (primal_pair, tangent_pair) = jvp_fn(loss_fn, 
            ...
            dvdt_detached = dvdt.detach()
            del dvdt
            d_dvdt_d_txt_detached = d_dvdt_d_txt.detach()
            del d_dvdt_d_txt

            v_trgt = v - (t - r_)[:, None, None, None] * dvdt_detached
            d_v_d_txt_trgt = d_v_d_txt * eps - (t - r_)[:, None, None, None] * d_dvdt_d_txt_detached

            error1 = v_pred - v_trgt
            error2 = d_v_pred_d_txt - d_v_d_txt_trgt


            loss1 = adaptive_l2_loss(error1, gamma=args.gamma, c=1e-3)  # use adaptive L2 loss
            loss2 = adaptive_l2_loss(error2, gamma=args.gamma, c=1e-3)  # use adaptive L2 loss
            if args.sob_lambda > 0:
                loss = loss1 + loss2 * args.sob_lambda
            else:
                loss = loss1
            ...
            epoch_derror += torch.mean(torch.square(dvdt_detached))
